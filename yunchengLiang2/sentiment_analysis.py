...
class BoldChatETL:
    ...
    def __sentiment_analysis_amazon(self, boldchat_df): 
        ...
        empty_df = spark.createDataFrame([['']]).toDF("extract_msg")
        pipelineModel = pipeline.fit(empty_df)#boldchat_df
        boldchat_df_col=list(boldchat_df.columns)
        boldchat_df_col.append("class")
        print(boldchat_df_col)
        return pipelineModel\
           .transform(boldchat_df).select(boldchat_df_col)
      
      
    def __sentiment_analysis_emotion(self, boldchat_df): 
        ...
        empty_df = spark.createDataFrame([['']]).toDF("extract_msg")
        pipelineModel = pipeline.fit(boldchat_df)#empty_df
        boldchat_df_col=list(boldchat_df.columns)
        boldchat_df_col.append("sentiment")
        print(boldchat_df_col)
        return pipelineModel\
           .transform(boldchat_df).select(boldchat_df_col)
    ...
    def cbl_ser_sentiment(self):
        return self.boldchat_df\
            ...
            .transform(self.__sentiment_analysis_amazon)
    def cbl_ser_emotion(self):
        return self.boldchat_df\
            .transform(self.__filter_by_channel('Cable Service EN'))\
            .transform(self.__inbound_type)\
            .transform(self.__chat_date)\
            ...
            .transform(self.__truncate_message)\
            .transform(self.__msg_not_empty("extract_msg"))\
            .transform(self.__sentiment_analysis_emotion)
...
# MAGIC %sql
# MAGIC REFRESH TABLE boldchat.boldchat_session
...
#cbl_ser_spark_sentiment = boldchat_etl.cbl_ser_sentiment()
cbl_ser_spark_emotion = boldchat_etl.cbl_ser_emotion()
...
# Process manually
input_list = [
    """hi i have transferred my cell phone to rogers last month and today i received the message that my bill is not paid, however i never received it. in my “myrogers” profile, it’s only listed my ignite tv and internet. could you please add my cell phone to my profile. ?? ——. ?? ok. ok. ———. ————- ————- —- ———- sure. no. hi i have transferred my cell phone to rogers last month and today i received the message that my bill is not paid, however i never received it. in my “myrogers” profile, it’s only listed my ignite tv and internet. could you please add my cell phone to my profile. i need my cell phone bill to be available in my profile. yes, please. okay, sure thing. no problem. okay thanks. and could i have it billed to my credit card as well? just like my tv and internet. okay. thanks. yes, it is there. thanks a lot, ganindu. very satisfied."""]
df = spark.createDataFrame(pd.DataFrame({"text": input_list}))
lresult = light_pipeline.fullAnnotate(input_list)
for example in lresult:
  for res in example['ner_chunk']:
...
# Process manually
input_list = [
    """hi, my roommate just transferred his number to me. however, i didn't receive any confirmation email nor can i find my user name with my email address. can you look into the issue for me? i'm okay, thank you. how about you. sure. yes, i think so. it's a home internet account. sure. i just submitted the form. thank you. great. yes. sounds good. i got the email. this should be good now? by the way, i log into my account and see a balance fee of *** .38. do you know what this is? yes the profile has been updated. thanks a lot! my roommate paid a bill of about 99$ right before transferring to me. was that the bill for july? okay, then i will check this later. when do you think it will be fully updated? sounds good. thanks a lot for your help! that's all, thanks again. you too bye."""]
df = spark.createDataFrame(pd.DataFrame({"text": input_list}))
lresult = light_pipeline.fullAnnotate(input_list)
for example in lresult:
  for res in example['ner_chunk']:
...
# Process manually
input_list = [
    """why is my internet charges so high compared to the last bill. morning i am inquiring as to why my internet charges are so high compared to last month. sounds good. not that i know of. there is a big difference in the amount like $50.92. i like the new amount of $***.14 so please sign me up for it. i was never notified about my promotion ending so i am very disappointed with rogers. we gave been with rogers since the 70s. sorry for the spelling i am trying to deal with 2annoying 2 annoying grand children. no i don't view my billing unless there is a different amount owing. seeing as we have been with rogers for so long is there any way to reduce my billing because of the internet charges. why is our cell phones nit included. please do not do anything with my services or billing at the moment as i am consideribg cancelling all services and going with a different provider. i am not very happy with the prices you are quoting me."""]
df = spark.createDataFrame(pd.DataFrame({"text": input_list}))
lresult = light_pipeline.fullAnnotate(input_list)
for example in lresult:
  for res in example['ner_chunk']:
...
# Process manually
input_list = [
    """i am doing well, you? i have been looking at the price of my services over the past few months. it has gone from approx $*** per month to this m onth it being $***. i would like to ask you one last time if there is anything you can do about the price change. okay. okay. erica, i have already looked at your competitors as an alternative. i can get a package that includes internet, phone and iptv for 2 hd tv for $***/mth. *** mbps, iptv package (same as ignite) with *** hd channels, free on demand movies and tv series. i have already installed the iptv service and have been trialing it for more than 6 mths. it has all the channels my roger packages offers, plus many more, such as hbo, sky network movies pacakge, movie channels, crave tv, i am currently paying $25/ mth for the servgice, to add my 2nd tv the monthly rate goes to $40/mth. the *** mbps service from your competitor, is more than enough to run the service. i also have a offer for home phone for $15 per month with 7 features and canada wide calling (voip) i have been a rogers customer for years. but i cannot justify the pricing i am now paying. but i wanted to give you guys a chance to get closer. okay. well give me your best price and i will make up my mind. sure. that is the service your competitor offers. 2. oh, one last thing the internet is on a "non" contract. no special price that increases 12 months later. ? why are my service increasing? the discounts i had from the last offer expired on july 8. it was a 24 mth term. i thought the last one was 24 mths. okay sorry. what is the price in 12 mths. erica, i am really tired of arguing with you guys every 12 mths. it really really pisses me off having to do that every year. let me think about your offer. one last thing, if i decide to go elsewhere, do i simply take all electronics back to the local rogers store? yes. thanks for the heads up. i was aware of the email thing, that is why we done use the supplier supplied domains. i already discussed the phone number porting with the new vendor. thanks for your help. no. i'm okay. you too."""]
df = spark.createDataFrame(pd.DataFrame({"text": input_list}))
lresult = light_pipeline.fullAnnotate(input_list)
for example in lresult:
  for res in example['ner_chunk']:
